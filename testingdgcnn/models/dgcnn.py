"""
DGCNN (Dynamic Graph CNN) for 3D Point Cloud Feature Extraction
Based on: "Dynamic Graph CNN for Learning on Point Clouds" (Wang et al., 2019)
"""

import torch
import torch.nn as nn
import torch.nn.functional as F


def knn(x, k):
    """
    Find k-nearest neighbors for each point
    
    Args:
        x: (batch_size, num_dims, num_points)
        k: number of neighbors
    
    Returns:
        idx: (batch_size, num_points, k)
    """
    inner = -2 * torch.matmul(x.transpose(2, 1), x)
    xx = torch.sum(x**2, dim=1, keepdim=True)
    pairwise_distance = -xx - inner - xx.transpose(2, 1)
 
    idx = pairwise_distance.topk(k=k, dim=-1)[1]   # (batch_size, num_points, k)
    return idx


def get_graph_feature(x, k=20, idx=None):
    """
    Construct edge features for graph convolution
    
    Args:
        x: (batch_size, num_dims, num_points)
        k: number of neighbors
        idx: neighbor indices (optional)
    
    Returns:
        feature: (batch_size, 2*num_dims, num_points, k)
    """
    batch_size = x.size(0)
    num_points = x.size(2)
    x = x.view(batch_size, -1, num_points)
    
    if idx is None:
        idx = knn(x, k=k)   # (batch_size, num_points, k)
    
    device = x.device
    
    idx_base = torch.arange(0, batch_size, device=device).view(-1, 1, 1) * num_points
    idx = idx + idx_base
    idx = idx.view(-1)
 
    _, num_dims, _ = x.size()
    
    x = x.transpose(2, 1).contiguous()   # (batch_size, num_points, num_dims)
    feature = x.view(batch_size * num_points, -1)[idx, :]
    feature = feature.view(batch_size, num_points, k, num_dims) 
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k, 1)
    
    feature = torch.cat((feature - x, x), dim=3).permute(0, 3, 1, 2).contiguous()
  
    return feature      # (batch_size, 2*num_dims, num_points, k)


class DGCNN(nn.Module):
    """
    Dynamic Graph CNN for 3D Knuckle Pattern Recognition
    
    Architecture:
    - 4 EdgeConv layers with increasing channels
    - Global max + avg pooling
    - MLP head for feature extraction
    
    Output: 256D feature vector for each point cloud
    """
    
    def __init__(self, k=20, emb_dims=256, dropout=0.5):
        super(DGCNN, self).__init__()
        self.k = k
        self.emb_dims = emb_dims
        
        # EdgeConv layers
        self.bn1 = nn.BatchNorm2d(64)
        self.bn2 = nn.BatchNorm2d(64)
        self.bn3 = nn.BatchNorm2d(128)
        self.bn4 = nn.BatchNorm2d(256)
        self.bn5 = nn.BatchNorm1d(emb_dims)

        self.conv1 = nn.Sequential(
            nn.Conv2d(6, 64, kernel_size=1, bias=False),
            self.bn1,
            nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(64*2, 64, kernel_size=1, bias=False),
            self.bn2,
            nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(64*2, 128, kernel_size=1, bias=False),
            self.bn3,
            nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(128*2, 256, kernel_size=1, bias=False),
            self.bn4,
            nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv5 = nn.Sequential(
            nn.Conv1d(512, emb_dims, kernel_size=1, bias=False),
            self.bn5,
            nn.LeakyReLU(negative_slope=0.2)
        )
        
        # Feature extraction head
        self.linear1 = nn.Linear(emb_dims*2, 512, bias=False)
        self.bn6 = nn.BatchNorm1d(512)
        self.dp1 = nn.Dropout(p=dropout)
        self.linear2 = nn.Linear(512, 256)
        
    def forward(self, x):
        """
        Forward pass
        
        Args:
            x: (batch_size, 3, num_points) - 3D point cloud
        
        Returns:
            features: (batch_size, 256) - feature vector
        """
        batch_size = x.size(0)
        
        # EdgeConv 1
        x = get_graph_feature(x, k=self.k)      # (batch_size, 6, num_points, k)
        x = self.conv1(x)                       # (batch_size, 64, num_points, k)
        x1 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 64, num_points)

        # EdgeConv 2
        x = get_graph_feature(x1, k=self.k)     # (batch_size, 128, num_points, k)
        x = self.conv2(x)                       # (batch_size, 64, num_points, k)
        x2 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 64, num_points)

        # EdgeConv 3
        x = get_graph_feature(x2, k=self.k)     # (batch_size, 128, num_points, k)
        x = self.conv3(x)                       # (batch_size, 128, num_points, k)
        x3 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 128, num_points)

        # EdgeConv 4
        x = get_graph_feature(x3, k=self.k)     # (batch_size, 256, num_points, k)
        x = self.conv4(x)                       # (batch_size, 256, num_points, k)
        x4 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 256, num_points)

        # Aggregate features
        x = torch.cat((x1, x2, x3, x4), dim=1)  # (batch_size, 512, num_points)

        x = self.conv5(x)                       # (batch_size, emb_dims, num_points)
        
        # Global pooling
        x1 = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)           # (batch_size, emb_dims)
        x2 = F.adaptive_avg_pool1d(x, 1).view(batch_size, -1)           # (batch_size, emb_dims)
        x = torch.cat((x1, x2), 1)              # (batch_size, emb_dims*2)

        # MLP head
        x = F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.2) # (batch_size, 512)
        x = self.dp1(x)
        x = self.linear2(x)                                             # (batch_size, 256)
        
        # L2 normalize
        x = F.normalize(x, p=2, dim=1)
        
        return x


if __name__ == "__main__":
    # Test the model
    print("="*60)
    print("Testing DGCNN Architecture")
    print("="*60)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\nDevice: {device}")
    
    # Create model
    model = DGCNN(k=20, emb_dims=256, dropout=0.5).to(device)
    
    # Count parameters
    num_params = sum(p.numel() for p in model.parameters())
    print(f"Parameters: {num_params:,}")
    
    # Test forward pass
    batch_size = 4
    num_points = 1024
    
    x = torch.randn(batch_size, 3, num_points).to(device)
    print(f"\nInput shape: {x.shape}")
    
    with torch.no_grad():
        features = model(x)
    
    print(f"Output shape: {features.shape}")
    print(f"Feature dimension: {features.shape[1]}D")
    
    print("\n✅ DGCNN model working correctly!")
    print("="*60)
